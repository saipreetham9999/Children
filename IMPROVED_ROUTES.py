from flask import Blueprint, request, jsonify, current_app
from bus.JoLogger import get_logger

bp = Blueprint("routes", __name__)
log = get_logger("BrainRoutes")


def get_brain():
    return current_app.brain


# ── existing ────────────────────────────────────────────────────────────
@bp.route("/")
def index():
    log.info("Index page accessed")
    brain = get_brain()
    children = brain.connection_manager.get_all()
    return jsonify({
        "brain": "online",
        "children_connected": len(children),
        "children": list(children.keys())
    })


@bp.route("/api/status", methods=["GET"])
def status():
    log.info("Status endpoint accessed")
    brain = get_brain()
    children = brain.connection_manager.get_all()
    return jsonify({
        "status": "online",
        "children": [
            {
                "device_name": c["device_name"],
                "device_type": c["device_type"],
                "capabilities": c["capabilities"],
            }
            for c in children.values()
        ]
    })


# ── new HTTP connection endpoints ────────────────────────────────────────
@bp.route("/api/connect", methods=["POST"])
def connect():
    """Child sends identity. Brain registers it."""
    try:
        identity = request.get_json()
        if not identity:
            log.warning("❌ Connect rejected: No JSON body")
            return jsonify({"status": "rejected", "reason": "no JSON body"}), 400

        device_name = identity.get("device_name", "unknown")
        log.info("📱 Connect request: %s", device_name)

        brain = get_brain()
        result = brain.connection_manager.handle_connect(identity)

        if result["status"] == "rejected":
            log.warning("❌ Connect rejected: %s", result.get("reason"))
            return jsonify(result), 400

        log.info("✅ Connect accepted: %s", device_name)
        return jsonify(result), 200

    except Exception as e:
        log.error("❌ Connect error: %s", str(e))
        return jsonify({"status": "error", "reason": str(e)}), 500


@bp.route("/api/heartbeat", methods=["POST"])
def heartbeat():
    """Child sends heartbeat every 5 seconds to stay alive."""
    try:
        data = request.get_json()
        if not data or "device_name" not in data:
            return jsonify({"status": "error", "reason": "device_name required"}), 400

        device_name = data["device_name"]
        brain = get_brain()
        result = brain.connection_manager.handle_heartbeat(device_name)

        if result["status"] == "unknown":
            log.warning("❌ Heartbeat from unknown: %s", device_name)
            return jsonify(result), 404

        return jsonify(result), 200

    except Exception as e:
        log.error("❌ Heartbeat error: %s", str(e))
        return jsonify({"status": "error", "reason": str(e)}), 500


@bp.route("/api/disconnect", methods=["POST"])
def disconnect():
    """Child cleanly disconnects."""
    try:
        data = request.get_json()
        if not data or "device_name" not in data:
            return jsonify({"status": "error", "reason": "device_name required"}), 400

        device_name = data["device_name"]
        log.info("🔌 Disconnect: %s", device_name)

        brain = get_brain()
        brain.connection_manager.handle_disconnect(device_name)
        return jsonify({"status": "disconnected"}), 200

    except Exception as e:
        log.error("❌ Disconnect error: %s", str(e))
        return jsonify({"status": "error", "reason": str(e)}), 500


@bp.route("/api/events", methods=["GET"])
def events():
    """Child polls for pending events/alerts."""
    try:
        device_name = request.args.get("device_name")
        if not device_name:
            log.warning("❌ Events poll: device_name required")
            return jsonify({"status": "error", "reason": "device_name required"}), 400

        brain = get_brain()
        pending = brain.message_router.pop_events(device_name)

        if pending:
            log.info("📨 Sending %d events to %s", len(pending), device_name)

        return jsonify({
            "status": "ok",
            "events": pending
        }), 200

    except Exception as e:
        log.error("❌ Events error: %s", str(e))
        return jsonify({"status": "error", "reason": str(e)}), 500


@bp.route("/api/report", methods=["POST"])
def report():
    """Child sends data report (camera frame, sensor data etc)."""
    try:
        log.info("📸 Report endpoint hit!")

        data = request.get_json()
        if not data:
            log.warning("❌ Report rejected: No JSON body")
            return jsonify({"status": "error", "reason": "no JSON body"}), 400

        device_name = data.get("device_name", "unknown")
        report_type = data.get("type", "unknown")
        has_data = "data" in data

        log.info("📊 Report from %s (type=%s, has_data=%s)", device_name, report_type, has_data)

        if report_type == "frame":
            data_size = len(data.get("data", ""))
            log.info("🎬 Frame report: %s bytes", data_size)

        brain = get_brain()
        brain.bus.publish("child.report.received", data)

        log.info("✅ Report received and published")
        return jsonify({"status": "received"}), 200

    except Exception as e:
        log.error("❌ Report error: %s", str(e))
        log.error("❌ Exception type: %s", type(e).__name__)
        return jsonify({"status": "error", "reason": str(e)}), 500


@bp.route("/api/health", methods=["GET"])
def health():
    """Returns health status of all services."""
    try:
        brain = get_brain()
        report = brain.health_checker.get_report() if hasattr(brain, 'health_checker') else {}
        all_healthy = all(s == "running" for s in report.values())

        status_code = 200 if all_healthy else 503
        return jsonify({
            "status": "healthy" if all_healthy else "degraded",
            "services": report,
            "ai_tier": brain.tier_manager.get_tier() if hasattr(brain, 'tier_manager') else "unknown",
            "ai_processor": brain.tier_manager.get_processor() if hasattr(brain, 'tier_manager') else "unknown",
        }), status_code

    except Exception as e:
        log.error("❌ Health error: %s", str(e))
        return jsonify({"status": "error", "reason": str(e)}), 500
